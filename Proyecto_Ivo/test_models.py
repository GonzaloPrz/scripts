...
for task in tasks:
    ...
    for dimension in dimensions:
        ...
        for y_label,hyp_opt,feature_selection,bootstrap in itertools.product(y_labels,hyp_opt_list,feature_selection_list,bootstrap_list):
            path_to_results = results_dir / task / dimension / scaler_name / 'all_features' /kfold_folder / f'{n_seeds_train}_seeds_train' / f'{n_seeds_test}_seeds_test' / y_label / 'no_hyp_opt' / 'feature_selection' / 'bootstrap'
            ...
            files = [file for file in Path(path_to_results).iterdir() if 'all_performances' in file.stem and 'test' not in file.stem]

            X_dev = pickle.load(open(Path(path_to_results,'X_dev.pkl'),'rb'))
            y_dev = pickle.load(open(Path(path_to_results,'y_dev.pkl'),'rb'))

            X_test = pickle.load(open(Path(path_to_results,'X_test.pkl'),'rb'))
            y_test = pickle.load(open(Path(path_to_results,'y_test.pkl'),'rb'))

            IDs_test = pickle.load(open(Path(path_to_results,'IDs_test.pkl'),'rb'))
