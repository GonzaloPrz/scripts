...
plt.tight_layout()
plt.savefig(Path(Path.home(), 'data', 'MPLS', 'reunion','age_distribution.png'))
...
plt.ylabel('Género')
plt.tight_layout()
plt.savefig(Path(Path.home(), 'data', 'MPLS','reunion','sex_distribution.png'))
...
plt.tight_layout()
plt.savefig(Path(Path.home(), 'data', 'MPLS','reunion','microconcert_participation.png'))
...
task_completion_df.to_csv(Path(Path.home(), 'data','MPLS','reunion','task_completion.csv'), index=False)
...
plt.ylabel('Frecuencia')
plt.tight_layout()
plt.savefig(Path(Path.home(), 'data','MPLS','reunion','minimental_distribution.png'))

mmse_counts = data['Minimental_class'].value_counts()
print("Cantidad de participantes con y sin deterioro cognitivo:")
print(mmse_counts)

# Generar gráfico de la cantidad de participantes por idioma
plt.figure(figsize=(10, 6))
mmse_counts.plot(kind='bar', color='pink', edgecolor='black')
plt.title('Cantidad de participantes por categoría de Minimental')
plt.ylabel('Cantidad de participantes')
plt.xlabel('Categoría')
plt.tight_layout()
plt.savefig(Path(Path.home(), 'data', 'MPLS','reunion','mmmse_distribution.png'))
...
if 'language' in data.columns:
    ...
    plt.xlabel('Idioma')
    plt.tight_layout()
    plt.savefig(Path(Path.home(), 'data', 'MPLS','reunion','language_distribution.png'))
else:
