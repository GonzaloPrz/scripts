...
for scoring in scoring_metrics:
    ...
    for y_label,task,dimension in itertools.product(y_labels,tasks,dimensions):
        ...
        model_index = best_models[(best_models['y_label'] == y_label) & (best_models['task'] == task) & (best_models['dimension'] == dimension)]['model_index'].values[0]
        model_type = best_models[(best_models['y_label'] == y_label) & (best_models['task'] == task) & (best_models['dimension'] == dimension)]['model_type'].values[0]
        random_seed = best_models[(best_models['y_label'] == y_label) & (best_models['task'] == task) & (best_models['dimension'] == dimension)]['random_seed_test'].values[0]
        ...
        calibrated_scores_ = pickle.load(open(Path(path_to_scores,f'outputs_{model_type}_calibrated.pkl'),'rb'))[0,model_index]
        ...
        filename_to_save = f"best_{task}_{dimension}_{model_type}_logpost.png"
        ...
        filename_to_save = f"best_calibrated_{task}_{dimension}_{model_type}_logpost.png"
        plot_hists(y_true, calibrated_scores, outfile=Path(results_dir,"plots",filename_to_save), nbins=50, group_by='score', style='--', label_prefix='calibrated ', axs=ax)