...
for scoring in scoring_metrics:
    ...
    for r, row in best_models.iterrows():
        ...
        overall_perf = metric(flatten_except(outputs,outputs.ndim-1).reshape(-1,2), y_true.flatten(), priors=deploy_priors, norm=True)
        overall_perf_after_cal = metric(flatten_except(cal_outputs,cal_outputs.ndim-1).reshape(-1,2), y_true.flatten(), priors=deploy_priors, norm=True)
