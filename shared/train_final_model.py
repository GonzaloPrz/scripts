...
for shuffle_labels in shuffle_labels_list:
    best_classifiers = pd.read_csv(Path(results_dir,f'best_classifiers_{scoring}_{kfold_folder}_{scaler_name}_hyp_opt_feature_selection_shuffled.csv'))  if shuffle_labels else pd.read_csv(Path(results_dir,f'best_classifiers_{scoring}_{kfold_folder}_{scaler_name}_hyp_opt_feature_selection.csv')) 
    ...
    for task,feature_selection in itertools.product(tasks,feature_selection_list):
        ...
        for dimension in dimensions:
            ...
            all_features = [col for col in best_classifier.columns if any(f'{x}_{y}__' in col for x,y in itertools.product(task.split('__'),dimension.split('__')))]
            ...
            if task == 'Animales' and dimension == 'properties__timing':
                print(all_features,features,set(all_features)-set(features))
