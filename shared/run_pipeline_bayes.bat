...
if "%~1"=="-feature_selection" (
    set feature_selection=%~2
    shift
    shift
    goto parse_args
)
...
if "%project_name%"=="" (
    echo Error: el parametro -project_name es obligatorio.
    goto :EOF
)
...
python "C:\Users\CNC Audio\gonza\scripts\shared\bootstrap_models_bayes.py" ^
    %project_name% %feature_selection% %filter_outliers% %shuffle_labels% %k%

:: Mostrar los parametros usados
