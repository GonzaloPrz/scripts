...
for condition in conditions:
    df = pd.read_csv(Path(data_dir,f'all_data_{condition}_matched_{condition}.csv'))
    for c in set(conditions) - set([condition]):
        df[c] = np.nan
    features = [ft for ft in df.columns if '__' in ft]
    if merged_dataset.empty:
        merged_dataset = df[['id','has_anxiety','has_depression'] + features]
    else:
        merged_dataset = pd.concat((merged_dataset, df[['id','has_anxiety','has_depression'] + features]),axis=0)
