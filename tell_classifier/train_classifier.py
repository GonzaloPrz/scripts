...
dimensions = ['pitch','voice-quality','talking-intervals',
              'pitch_talking-intervals','pitch_voice-quality','talking-intervals_voice-quality',
              'pitch_talking-intervals_voice-quality']
...
data_dir = Path(Path.home(),'data','tell_classifier') if 'Users/gp' in str(Path.home()) else Path('D:','data','tell_classifier')
...
for y_label,task,dimension in itertools.product(y_labels,tasks,dimensions):
    ...
    data = pd.read_excel(Path(data_dir,f'data_{task}.xlsx'))
    ID = data[id_col]
    ...
    if '_' in dimension:
        all_features = [col for col in data.columns if any(x in col for x in dimension.split('_'))]
    else:
        all_features = [col for col in data.columns if dimension in col]    
    ...
    for hyp_tuning,model in itertools.product(hyp_tuning_list,models_dict.keys()):        
        ...
        path_to_save = Path(data_dir.parent.parent,'results','tell_classifier',task,dimension,scaler_name,kfold_folder,f'{n_seeds_train}_seeds_train',f'{n_seeds_test}_seeds_test',y_label,'no_hyp_opt','feature_selection')
        ...
        random_seeds_test = np.arange(1,2)
