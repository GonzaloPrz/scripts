...
tasks = ['MOTOR_LIBRE__NEUTRO_LECTURA']
scoring = 'roc_auc'
...
for y_label,task in itertools.product(y_labels,tasks):                        
    ...
    if 'task' in data.columns:
        data.drop(columns='task',inplace=True)
    ...
    for hyp_tuning,feature_selection,calibrate,model in itertools.product(hyp_tuning_list,feature_selection_list,calibrate_list,models_dict.keys()):
        ...
        if Path(path_to_save / f'results_val_{filename_to_save}.csv').exists():
            continue

        all_results = pd.DataFrame(columns=['random_seed']) 

        IDs_train = pd.DataFrame(columns=['random_seed','ID'])
