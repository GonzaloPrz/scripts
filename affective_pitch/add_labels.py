...
for groups in[['CN','AD'],['CN','FTD'],['AD','FTD']]:
    ...
    subset.to_csv(Path(base_dir.parent,f'affective_pitch_{group_label}',f'all_data_{group_label}.csv'),index=False)

data['group'] = data['group'].map({'CN':0,'AD':1,'FTD':2})
data = data.drop([col for col in data.columns if 'Unnamed' in col],axis=1)

data.to_csv(Path(base_dir,'all_data.csv'),index=False)