...
labels = pd.read_csv(data_dir / 'transcripts_fugu_matched_group_sentiment_windows.csv')

all_data = labels

for sentiment in ['ALL','POS','NEG','NEU']:
    pitch_data = pd.read_csv(data_dir / 'divided_audios' / 'windows' / sentiment / f'{sentiment}_energy_pitch_features_windows.csv')
    ...
    features = np.unique([col.split('__')[3] for col in pitch_data.columns if col.startswith('Fugu__window')])
    ...
    mean_data.to_csv(Path(data_dir,f'mean_{sentiment}_pitch_features_windows.csv'),index=False)

    all_data = pd.merge(all_data,mean_data,on='id',how='left')

all_data.to_csv(data_dir / 'mean_pitch_features_windows.csv',index=False)